...
export function getList() {
    const request = axios.get(`${process.env.REACT_APP_API_HOST}/measure_units`)
    return {
        type: 'MEASURE_UNITS_FETCHED',
        payload: request
    }
}
...
function submit(context, values, method) {
    return dispatch => {
        ...
        axios[method](`${process.env.REACT_APP_API_HOST}/measure_units${id ? '/'+id : ''}`, filteredValues)
        .then(resp => {
...
export function init(item = INITIAL_VALUES) {
    return initialize('measureUnitForm', item)
}

// REDUCER
export function measureUnitReducer (state = INITIAL_STATE, action) {
    switch(action.type) {
        case 'MEASURE_UNITS_FETCHED':
